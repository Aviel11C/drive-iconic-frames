...
export function VehicleCard({ vehicle }: { vehicle: Vehicle }) {
  return (
    <article className="group relative overflow-hidden bg-card border border-border/60 hover:border-gold/50 transition-all duration-700">
      ...
      <div className="px-7 pb-7 -mt-3 flex items-center justify-between">
        <Link
          to="/collection/$slug"
          params={{ slug: vehicle.slug }}
          className="inline-flex items-center gap-2 text-[11px] tracking-luxury uppercase text-foreground/85 hover:text-gold transition-colors duration-500 group/link"
        >
          View Details
        ...
        <Link
          ...
          className="text-[10px] tracking-luxury uppercase text-gold/80 hover:text-gold transition-colors"
        >
          Reserve →
        </Link>
      </div>
